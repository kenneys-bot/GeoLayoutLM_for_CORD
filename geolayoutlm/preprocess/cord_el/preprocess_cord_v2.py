...
from json_cord2funsd import json_cord2funsd
import argparse

parser = argparse.ArgumentParser(
    description='Process CORD2FUNSD'
)
parser.add_argument(
    '--inputpath', type=str, help='传入CORD地址', default="./CORD/drive-download-20230918T081933Z-001"
)
parser.add_argument(
    '--outputpath', type=str, help="传入cord输出地址", default="./CORD/drive-download-20230918T081933Z-001/cord_geo"
)
parser.add_argument(
    '--voca', type=str, help="传入模型路径", default="./hlf/bert-base-uncased"
)
parser.add_argument(
    '--model_type', type=str, help="模型类型", default="bert"
)
...
CLASSES = ["O", "menu.cnt", "menu.nm", "menu.price", "sub_total.subtotal_price", "sub_total.service_price",
           "sub_total.tax_price", "sub_total.etc", "total.total_price", "menu.sub_nm", "total.cashprice",
           "total.changeprice", "total.menutype_cnt", "total.menuqty_cnt", "sub_total.discount_price", "menu.unitprice",
           "total.total_etc", "total.creditcardprice", "menu.num", "menu.sub_cnt", "menu.discountprice",
           "menu.sub_price", "total.emoneyprice", "menu.sub_unitprice", "menu.sub_etc", "void_menu.nm",
           "void_menu.price", "sub_total.othersvc_price", "menu.vatyn", "menu.itemsubtotal", "menu.etc"]
CLASSES_VALID = ["menu.cnt", "menu.nm", "menu.price", "sub_total.subtotal_price", "sub_total.service_price",
           "sub_total.tax_price", "sub_total.etc", "total.total_price", "menu.sub_nm", "total.cashprice",
           "total.changeprice", "total.menutype_cnt", "total.menuqty_cnt", "sub_total.discount_price", "menu.unitprice",
           "total.total_etc", "total.creditcardprice", "menu.num", "menu.sub_cnt", "menu.discountprice",
           "menu.sub_price", "total.emoneyprice", "menu.sub_unitprice", "menu.sub_etc", "void_menu.nm",
...
def do_preprocess(tokenizer, dataset_split):
    ...
    for json_file in tqdm(json_files):
        ...
        # CORD's json to FUNSD's json
        in_json_obj = json_cord2funsd(in_json_obj)
        ...
        # meta
        out_json_obj['meta'] = {}
        image_file = (os.path.splitext(json_file.replace(f"/{anno_dir}/", "/images/"))[0] + ".png")
        ...
        if dataset_split == "train":
            ...
            out_json_obj['meta']['image_path'] = image_file[image_file.find("dev/"):]
